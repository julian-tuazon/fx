...
app.get('/api/reviews', (req, res, next) => {
  ...
  db.query(reviews, values)
    .then(result => {
      ...
      if (!review) {
        return res.json({ note: null, restaurantName: req.query.restaurantName, thumbsRate: null, yelpId: req.query.yelpId });
      }
      res.json(review);
    }
    )
...
app.get('/api/reviews/:yelpId', (req, res, next) => {
  ...
  db.query(reviews, userInfo)
    .then(result => {
      ...
      if (!reviews) {
        return res.json({ notes: null, thumbsRate: null, yelpId: req.params.yelpId, restaurantName: req.params.restaurantName });
      }
...
app.get('/api/liked', (req, res, next) => {
  ...
  return db.query(getLikedRestaurantYelpIds, [userId])
    ...
    .then(likedRestaurants => {
      const likedRestaurantsWithYelpIdKeys = {};
      for (const key in likedRestaurants) {
        const restaurant = likedRestaurants[key];
        likedRestaurantsWithYelpIdKeys[restaurant.id] = restaurant;
      }
      return res.json(likedRestaurantsWithYelpIdKeys);
    })
